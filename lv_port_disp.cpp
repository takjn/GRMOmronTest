...
static void set_pixel(int x, int y, uint32_t color) {
    int i = (((LCD_PIXEL_WIDTH * 2) + 31u) & ~31u) * y + x * 2;
    uint8_t a = (color & 0xF0000000)>>28;
    uint8_t r = (color & 0x00F00000)>>20;
    uint8_t g = (color & 0x0000F000)>>12;
    uint8_t b = (color & 0x000000F0)>>4;
    fbuf_sensor_result[i + 0] = (a<<4 | r); // AR
    fbuf_sensor_result[i + 1] = (g<<4 | b); // GB
}

/* Initialize your display and the required peripherals. */
static void disp_init(void)
{
    // nothing to do (main.cpp will initilize)
}
...
static void disp_flush(lv_disp_drv_t * disp_drv, const lv_area_t * area, lv_color_t * color_p)
{
    ...
    dcache_clean(fbuf_sensor_result, sizeof(fbuf_sensor_result));
